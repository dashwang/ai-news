...
- **新增热标题内容置顶：标题对应的第一篇内容排序到各板块开头**
- **新增开场白后置顶：标题对应的正文内容会重新排序到各板块开头**
...
10. **去重：必须比对历史发布，避免重复**
11. **动态标题：每次根据当日热点生成**
12. **热标题内容置顶：每个板块的标题对应的那篇内容，要排序到该板块的最前面**
...
**问题**：动态标题对应的热点新闻，正文内容经常在各板块很深的位置，读者需要翻很久才能找到。

**解决**：把每个板块中与动态标题最相关的那篇内容，**重新排序到该板块的最前面**。

### 置顶规则：

1. **分析动态标题**：标题是"Replit Agent 4来袭"，找出各板块中与Replit Agent 4最相关的那篇
2. **置顶处理**：把那篇内容移到板块开头（位置#1）
3. **原位置内容顺移**：原来的#1顺移到#2，以此类推

### 示例：

原始顺序：
1. 3D-Knitting
2. Avoiding Trigonometry
3. s@ 去中心化
4. Returning to Rails
5. SBCL Lisp

动态标题："🔥 Replit Agent 4来袭"

置顶后（Product Hunt板块）：
1. Runner AI（AI原生电商）- 与"AI编程/Agent"相关 → **置顶**
2. Huddle01 Cloud
3. HTML Pub
4. Clawther
5. Prava
