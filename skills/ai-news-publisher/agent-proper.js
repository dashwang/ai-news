...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log('⚠️  Running in standalone mode (using Kilo fallback)');
  ...
  // Create mock agent with Kilo fallback
  ...
  const mockAgent = {
    ...
    llm: {
      chat: async ({ messages }) => {
        ...
        try {
          const result = execSync(`echo "${prompt.substring(0, 300).replace(/["`$]/g, '')}" | kilo 2>&1`, {
            encoding: 'utf8', timeout: 25000, stdio: ['pipe','pipe','pipe']
          });
          const out = result.trim();
          // If kilo output seems like an error, throw to trigger fallback
          if (out.includes('Error') || out.includes('error') || out.length < 2) {
            throw new Error('Kilo fallback failed');
          }
          return { content: out };
        } catch(e) {
