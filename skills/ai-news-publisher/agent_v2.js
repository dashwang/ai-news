#!/usr/bin/env node
/**
 * AI News Publisher - Agent Skill (Publish Only) - V2
 * Improved layout: tighter spacing, doubled content, no summary text
 */
...
function generateWechatHTML(news, hl) {
  ...
  const fmt = (name, items) => {
    ...
    return `<div style="margin:3px 0"><h3 style="font-size:15px;font-weight:800;margin:0 0 5px;color:${color};border-left:3px solid ${color};padding-left:8px">${name}</h3>${items.map((n, idx) => `
      <div style="margin-bottom:3px;padding-bottom:3px;border-bottom:1px solid #f5f5f5">
        <div style="font-size:15px;font-weight:700;line-height:1.65;color:#111;margin-bottom:4px">
          ${idx+1}. ${n.title_zh || n.title}
        </div>
        <div style="font-size:13px;color:#444;line-height:1.75;margin-top:2px">
          ${(n.summary_zh || '').substring(0, 280)}
        </div>
      </div>
    `).join('')}</div>`;
  };
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>北美AI圈日报 ${date}</title><style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;line-height:1.6;color:#333;margin:0;padding:0;background:#fff}.header{text-align:center;padding:12px 8px 8px;border-bottom:1px solid #e65100;margin-bottom:8px}.headline{font-size:19px;font-weight:800;color:#111;margin:6px 0 8px;line-height:1.4}.headline-img{width:100%;max-width:600px;height:auto;border-radius:8px;margin:0 auto 6px;display:block}.meta{font-size:10px;color:#888;margin-top:4px}.section{margin:3px 0}</style></head><body><div class="header"><div class="headline">${hlText}</div><img src="${img}" class="headline-img" alt="头条配图"><div class="meta">${date} · 共${news.length}条</div></div>
${body}</body></html>`;
}
...
function loadNews(date) {
  ...
  for (const n of data.news) {
    ...
    if (!n.summary_zh) n.summary_zh = n.summary?.substring(0,280) || '';
  }
  return data.news;
}
...
if (process.argv[1] && process.argv[1].includes('agent.js')) {
  run({ publish: process.argv.includes('--publish') }).then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
}
